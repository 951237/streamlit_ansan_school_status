...
### 3. GitHub Actions SSL 오류 대응 ✅
- **작업일**: 2025-11-09
- **구현 내용**:
  - `crawl_data.py`의 HTTPS 세션 어댑터를 `LegacySSLAdapter`로 교체해 TLSv1 이상과 낮은 시큐리티 레벨(`DEFAULT:@SECLEVEL=1`)을 허용.
  - 교육청 서버가 제공하는 구형 암호화 스택과의 핸드셰이크 실패(`SSLError: sslv3 alert handshake failure`)를 방지하고, 프록시/직접 연결 모두 동일한 SSL 컨텍스트를 사용하도록 정리.
- **확인 방법**:
  - 로컬에서 `python crawl_data.py` 실행(네트워크 허용 환경) 혹은 GitHub Actions 워크플로우를 수동 실행해 성공 여부 확인.

### 4. 상시 접근 강화를 위한 배포 가이드 ✅
- **작업일**: 2025-11-09
- **구현 내용**:
